...
for(i in 1:length(files))
{
  ...
  print("**********************************************************************************************")
  res_opt <- hdbscan(t(expr), minPts = minPts)
  print(res_opt)
  ...
  print("**********************************************************************************************")
  kmeans_cluster <- kmeans(t(expr), N_clust)
  ...
  print("**********************************************************************************************")
  GMM_cluster <- Mclust(t(expr), N_clust)
  summary(GMM_cluster)
  ...
  print("**********************************************************************************************")
  sc <- specc(t(expr), centers=N_clust)
  ...
  print("**********************************************************************************************")
  sce <- SingleCellExperiment(assays = list(counts = t(t(expr) + 50)))
  rowData(sce)$feature_symbol <- colnames(t(expr))
  logcounts(sce) <- t(log(t(expr) + 50))
  ...
  maxK=N_clust
  reps=200
  results<-ConsensusClusterPlus(as.matrix(expr), maxK=maxK, reps=reps, pItem=0.8, pFeature=1, 
  ...
  SNN(data = t(expr), outfile = "snn-cliq.txt", k = par.k, distance = distan)
  ...
  seurat_obj
  seurat_obj<-FindClusters(object = seurat_obj, genes.use=colnames(data), resolution=1.2, print.output=0, save.SNN=TRUE)
