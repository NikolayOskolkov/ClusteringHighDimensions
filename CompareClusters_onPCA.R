...
for(i in 1:length(files))
{
  ...
  print("**********************************************************************************************")
  res_opt <- hdbscan(PC$x[,1:optPC], minPts = minPts)
  print(res_opt)
  ...
  plot(opt_tsne, main="", col=colors[as.character(res_opt$cluster)], xlab="tSNE1", ylab="tSNE2")
  mtext("HDBSCAN")
  
  # 2) KMEANS ON TSNE
  print("PERFORMING KMEANS CLUSTERING")
  print("**********************************************************************************************")
  kmeans_cluster <- kmeans(PC$x[,1:optPC], N_clust)
  print(table(kmeans_cluster$cluster))
  plot(opt_tsne,main="", col=colors[as.numeric(as.character(kmeans_cluster$cluster))], xlab="tSNE1", ylab="tSNE2")
  mtext("KMEANS")
  
  # 3) GAUSSIAN MIXTURE MODEL ON TSNE
  print("PERFORMING GAUSSIAN MIXTURE MODEL CLUSTERING")
  print("**********************************************************************************************")
  GMM_cluster <- Mclust(PC$x[,1:optPC], N_clust)
  summary(GMM_cluster)
  plot(opt_tsne,main="", col=colors[as.numeric(as.character(GMM_cluster$classification))], xlab="tSNE1", ylab="tSNE2")
  mtext("GAUSSIAN MIXTURE MODEL")
  
  # 4) HIERARCHICAL CLUSTERING
  print("PERFORMING HIERARCHICAL CLUSTERING")
  print("**********************************************************************************************")
  dist.euc<-dist(PC$x[,1:optPC])
  hcl.euc<-hclust(dist.euc, method="ward.D2")
  clusters.euc<-cutree(hcl.euc, N_clust)
  print(table(clusters.euc))
  plot(opt_tsne,main="", col=colors[as.numeric(as.character(clusters.euc))], xlab="tSNE1", ylab="tSNE2")
  mtext("HIERARCHICAL CLUSTERING")
  
  # 5) SPECTRAL CLUSTERING
  print("PERFORMING SPECTRAL CLUSTERING")
  print("**********************************************************************************************")
  sc <- specc(PC$x[,1:optPC], centers=N_clust)
  print(table(as.numeric(sc)))
  plot(opt_tsne,main="", col=colors[as.numeric(as.character(as.numeric(sc)))], xlab="tSNE1", ylab="tSNE2")
  mtext("SPECTRAL CLUSTERING")
  
  # 6) SC3
  print("PERFORMING SC3 CLUSTERING")
  print("**********************************************************************************************")
  sce <- SingleCellExperiment(assays = list(counts = t(PC$x[,1:optPC] + 50)))
  rowData(sce)$feature_symbol <- colnames(PC$x[,1:optPC])
  logcounts(sce) <- t(log(PC$x[,1:optPC] + 50))
  sce <- sc3(sce, ks = N_clust, gene_filter=FALSE)
  print(table(as.numeric(colData(sce)[,1])))
  plot(opt_tsne, main="", col=colors[as.numeric(as.character(as.numeric(colData(sce)[,1])))], xlab="tSNE1", ylab="tSNE2")
  mtext("SC3")
  ...
  maxK=N_clust
  reps=200
  results<-ConsensusClusterPlus(as.matrix(t(PC$x[,1:optPC])), maxK=maxK, reps=reps, pItem=0.8, pFeature=1, 
                                title="Consensus Clustering",clusterAlg="km", distance="euclidean", seed=123, plot="png")
  plot(opt_tsne, main="", col=colors[as.numeric(as.character(results[[N_clust]][["consensusClass"]]))], xlab="tSNE1", ylab="tSNE2")
  ...
  SNN(data = PC$x[,1:optPC], outfile = "snn-cliq.txt", k = par.k, distance = distan)
  ...
  plot(opt_tsne, main="", col=colors_snn_cliq[as.numeric(as.character(as.numeric(snn.res[,1])))], xlab="tSNE1", ylab="tSNE2")
  mtext("SNN-CLIQ")
  ...
  seurat_obj
  seurat_obj<-FindClusters(object = seurat_obj, genes.use=colnames(data), resolution=0.5, print.output=0, save.SNN=TRUE)
  ...
  plot(opt_tsne, main="", col=colors_seurat[as.numeric(as.character(as.numeric(seurat_obj@ident)))], xlab="tSNE1", ylab="tSNE2")
  mtext("SEURAT")
